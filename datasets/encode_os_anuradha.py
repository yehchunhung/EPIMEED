...
dataset_name = 'os'
version_name = 'os-calm-rest'

for file_name in ['test', 'valid', 'train']:

    print(file_name)

    with smart_open.open('s3://'+dataset_name+'-xenbot/6.new-version/'+version_name+'/'+file_name+'.csv', 'rt', encoding='utf-8', transport_params={'session': session}) as infile:
            ...
            if prev_id != '' and cur_id != prev_id:
                ...
                if len(current_dialog) >= 2:
                    ...
                    for k in range(1, len(current_dialog)):
                        ...
                        uttr_ids = tokenizer.encode(current_dialog[k][2])
                    ...
                    if break_point > 1:
                        ...
                        for k in range(break_point):
                            f_uttr.write('{} | {}\n'.format(dialog_id, current_dialog[k][2]))
                            uttr_emots.append(ED_emotions.index(current_dialog[k][4]))
