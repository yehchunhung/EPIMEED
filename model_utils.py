...
def build_plain_model(model, max_length, vocab_size):
    inp = np.ones((1, max_length), dtype = np.int64)
    inp[0,:max_length//2] = np.random.randint(2, vocab_size, size = max_length//2)
    seg = np.zeros((1, max_length), dtype = np.int64)
    tar = np.ones((1, max_length), dtype = np.int64)
    ...
    _, _ = model(inp, seg, tar, seg, True, enc_padding_mask, combined_mask, dec_padding_mask)
...
def build_meed_plus_model(model, max_length, vocab_size):
    inp = np.ones((1, max_length), dtype = np.int32)
    inp[0,:max_length//2] = np.random.randint(2, vocab_size, size = max_length//2)
    seg = np.zeros((1, max_length), dtype = np.int32)
    tar = np.ones((1, max_length), dtype = np.int32)
    tar[0,:max_length//2] = np.random.randint(2, vocab_size, size = max_length//2)
    inp_emot = np.zeros((1, max_length), dtype = np.int32)
    inp_comm = np.zeros((1, max_length), dtype = np.int32)
    tar_emot = np.zeros(1, dtype = np.int32)
    tar_comm = np.zeros(1, dtype = np.int32)

    inp = tf.constant(inp)
    seg = tf.constant(seg)
    tar = tf.constant(tar)
    inp_emot = tf.constant(inp_emot)
    inp_comm = tf.constant(inp_comm)
    tar_emot = tf.constant(tar_emot)
    tar_comm = tf.constant(tar_comm)

    enc_padding_mask, combined_mask, dec_padding_mask = create_masks(inp, tar)
    _, _ = model(inp, seg, inp_emot, inp_comm, tar, seg, tar_emot, tar_comm, True, enc_padding_mask, combined_mask, dec_padding_mask)
...
def build_emo_pred_plus_model(model, max_length, vocab_size):
    inp = np.ones((1, max_length), dtype = np.int32)
    inp[0,:max_length//2] = np.random.randint(2, vocab_size, size = max_length//2)
    seg = np.zeros((1, max_length), dtype = np.int32)
    ...
    inp = tf.constant(inp)
    seg = tf.constant(seg)
    inp_emot = tf.constant(np.zeros((1, max_length), dtype = np.int32))
    inp_comm = tf.constant(np.zeros((1, max_length), dtype = np.int32))

    enc_padding_mask = create_padding_mask(inp)
    _ = model(inp, seg, inp_emot, inp_comm, True, enc_padding_mask)
