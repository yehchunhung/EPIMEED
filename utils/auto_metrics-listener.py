...
def cal_bleu(read_path, n, indices_arr):
    all_weights = [(1.0,), (0.5,0.5), (1/3,1/3,1/3), (0.25,0.25,0.25,0.25)]
    df = pd.read_csv(read_path)
    #references = df['tar_y'].tolist()
    references = (df.iloc[indices_arr])['tar_y'].tolist()
    #candidates = df['pred_y'].tolist()
    candidates = (df.iloc[indices_arr])['pred_y'].tolist()
    bleu_scores = []
...
def cal_dist(read_path, n, indices_arr):
    df = pd.read_csv(read_path)
    #candidates = df['pred_y'].tolist()
    candidates = (df.iloc[indices_arr])['pred_y'].tolist()
    all_grams = []
...
def cal_rouge(read_path, indices_arr):
...
def cal_avg_length(read_path, indices_arr):
    df = pd.read_csv(read_path)
    #candidates = df['pred_y'].tolist()
    candidates = (df.iloc[indices_arr])['pred_y'].tolist()
...
def cal_embed_sim(read_path, indices_arr):
    df = pd.read_csv(read_path)
    #references = df['tar_y'].tolist()
    references = (df.iloc[indices_arr])['tar_y'].tolist()
    #candidates = df['pred_y'].tolist()
...
def cal_multiple(read_path, indices_arr):
    df = pd.read_csv(read_path)
    #references = df['tar_y'].tolist()
    references = (df.iloc[indices_arr])['tar_y'].tolist()
    #candidates = df['pred_y'].tolist()
...
with open('./auto-metrics/auto-metrics-listener-4turns.csv', 'a') as infile:
    ...
    for model_name in ['xenbot-os-dramatic-rest', 'xenbot-os-dramatic-rest-ed', 'xenbot-os-dramatic-rest-osed-dramatic']:
        for dataset in ['os-dramatic-rest', 'osed-dramatic', 'ed']:
            simple_met = False
            complex_met = False
            #for version in ['ground', 'plain', 'alexa', 'meed2', 'simple', 'complex', 'simple', 'complex']:
            for version in ['ground', 'plain', 'alexa', 'meed2', 'simple', 'complex', 'simple', 'complex']:
            #for version in ['plain', 'alexa']:
                ...
                if version == 'meed2':
                    ...
                    read_path = './prediction-listener/{}/{}/{}-{}.csv'.format(dataset, version, model_name.replace('xenbot', 'plain'), version)
                elif version == 'alexa':
                    read_path = './prediction-listener/{}/{}/{}-{}.csv'.format(dataset, version, model_name, version)
                elif version == 'simple':
                    if simple_met == False:
                        method = 'argmax'
                        simple_met = True
                        ...
                        method = 'prob-sampled'
                    read_path = './prediction-listener/{}/{}-heldout/{}-{}.csv'.format(dataset, method, model_name, version)
                    ...
                    read_path = './prediction-listener/{}/argmax-heldout/{}-{}.csv'.format(dataset, model_name, version)
                ...
                d1 = cal_dist(read_path, 1, indices_arr)
                d2 = cal_dist(read_path, 2, indices_arr)            
                #bleu1 = cal_bleu(read_path, 1)
                #bleu2 = cal_bleu(read_path, 2)
                #ses = cal_embed_sim(read_path)
                metrics_dict = cal_multiple(read_path, indices_arr)
                avg_words = cal_avg_length(read_path, indices_arr)
                ...
                if version == 'simple' or version == 'complex':
                    ...
                    new_row = [model_name, dataset, version, d1, d2] + metrics_arr + [avg_words]
