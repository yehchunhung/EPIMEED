...
class MEEDPlusEmbedder(tf.keras.Model):
    def __init__(self, d_model, dropout_rate, layer_norm_eps,
        ...
        self.seg_embeddings = tf.keras.layers.Embedding(type_vocab_size, d_model, name = 'seg_embed')
        self.emot_embeddings = tf.keras.layers.Embedding(num_emotions, d_model, name = 'emot_embed')
        self.comm_embeddings = tf.keras.layers.Embedding(num_comms, d_model, name = 'comm_embed')

        self.layernorm = tf.keras.layers.LayerNormalization(epsilon = layer_norm_eps,
            name = 'layernorm_embed')
        self.dropout = tf.keras.layers.Dropout(dropout_rate, name = 'dropout_embed')

    def call(self, x, seg, emot, comm, training):
        # x.shape == (batch_size, seq_len)
        # seg.shape == (batch_size, seq_len)
        # emot.shape == (batch_size, seq_len), emoji ids
...
class MEEDPlus(tf.keras.Model):
    ...
    def call(self, inp, inp_seg, inp_emot, inp_comm, tar, tar_seg, tar_emot, tar_comm, training,
             enc_padding_mask, look_ahead_mask, dec_padding_mask):
        # inp_embed.shape == (batch_size, input_seq_len, d_model)
        ...
        # tar_embed.shape == (batch_size, target_seq_len, d_model)
        tar_embed = self.embedder(tar, tar_seg, tf.expand_dims(tar_emot, 1), tf.expand_dims(tar_comm, 1), training)
    ...
    def encode(self, inp, inp_seg, inp_emot, inp_comm, training, enc_padding_mask):
        # inp_embed.shape == (batch_size, input_seq_len, d_model)
        inp_embed = self.embedder(inp, inp_seg, inp_emot, inp_comm, training)
    ...
    def decode(self, enc_output, pred_tar_emot, tar_comm, tar, tar_seg, training, look_ahead_mask, dec_padding_mask):
        # tar_embed.shape == (batch_size, target_seq_len, d_model)
        tar_embed = self.embedder(tar, tar_seg, tf.expand_dims(pred_tar_emot, 1), tar_comm, training)
